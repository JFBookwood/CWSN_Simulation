"""
Export Cobaya likelihood for VGCF voids RSD/AP analysis.
"""
...
def main():
    ...
    final_yaml = yaml_template.format(
        example_yaml=example_yaml_path.replace("\\", "/"),
        data_file=data_npz.replace("\\", "/"),
        output_dir=os.path.join(OUT_DIR, "chains").replace("\\", "/"),
    )
    ...
    final_yaml_cosmo = yaml_cosmo.format(
        data_file=data_npz.replace("\\", "/"),
        output_dir=os.path.join(OUT_DIR, "chains_cosmo").replace("\\", "/"),
    )
