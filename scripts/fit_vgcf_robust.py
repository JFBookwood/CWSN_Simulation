"""
Fit the VGCF 2D correlation function grid to extract cosmological parameters.
"""
...
def smooth_template(s, xi0):
    try:
        ...
        return savgol_filter(
            xi0, window_length=max(5, win), polyorder=poly, mode="interp"
        )
    except Exception:
        k = max(3, len(xi0) // 10)
        k = k + 1 - k % 2
...
def interp_template(s_centers, xi0_values):
    ...
    def f(s):
        s = np.asarray(s)
        return np.interp(
            s, s_centers, xi0_values, left=xi0_values[0], right=xi0_values[-1]
        )
...
def main():
    ...
    try:
        ...
        print(
            f"[Error] SciPy not available ({e}). Please install scipy to run the fit."
        )
        return
    ...
    print(
        f"Robust fit: A={A:.3f}, beta={beta:.3f}, alpha_par={a_par:.4f}, alpha_perp={a_perp:.4f}, C={C:.4f}"
    )
    J = res.jac
    try:
    ...
    out = {
        ...
        "errors": {
            "A": float(perr[0]),
            "beta": float(perr[1]),
            "alpha_par": float(perr[2]),
            "alpha_perp": float(perr[3]),
            "C": float(perr[4]),
        },
        "unit": unit,
        "smin_fit": float(args.smin_fit),
        "smax_fit": float(args.smax_fit),
