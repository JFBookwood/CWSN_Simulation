"""
Convert DESI EDR ELG_HIP randoms from RA,DEC,Z to Cartesian XYZ coordinates.
"""
...
def list_random_files():
    ...
    if not files:
        raise FileNotFoundError(
            "No random FITS files found. Make sure to run the download script for randoms."
        )
    return files
...
def main():
    ...
    ap.add_argument(
        "--mpch", action="store_true", help="Write outputs in Mpc/h instead of Mpc"
    )
    ap.add_argument(
        "--workers", type=int, default=8, help="Parallel readers for FITS (>=1)"
    )
    ap.add_argument(
        "--min_ratio",
        type=float,
        default=5.0,
        help="Hard-fail if Nrand/Ndata < this (when data available)",
    )
    ap.add_argument(
        "--max_ratio",
        type=float,
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Path to script that can re-download missing/corrupt random tiles",
    )
    ap.add_argument(
        "--redownload_args",
        type=str,
        default="",
        help="Extra args passed to re-download script",
    )
    args = ap.parse_args()
    ...
    print(
        f"Randoms rows: before z-cut={n_before:,}, after z-cut={n_after:,} (kept {kept_frac:.2%})"
    )
    data_rows = None
    if os.path.exists(DATA_TXT):
        data_rows = sum(
            (1 for _ in open(DATA_TXT, "r", encoding="utf-8", errors="ignore"))
        )
    elif os.path.exists(DATA_CSV):
        data_rows = max(
            0,
            sum((1 for _ in open(DATA_CSV, "r", encoding="utf-8", errors="ignore")))
            - 1,
        )
    if data_rows and data_rows > 0:
        dr_ratio = n_after / data_rows
        print(
            f"Data→Randoms ratio (Nrand/Ndata) ≈ {dr_ratio:.2f} (data={data_rows:,}, rand={n_after:,})"
        )
        if dr_ratio < args.min_ratio or dr_ratio > args.max_ratio:
            print(
                f"[Error] Nrand/Ndata={dr_ratio:.2f} is outside allowed bounds [{args.min_ratio:.2f}, {args.max_ratio:.2f}]."
            )
            if SKIPPED_FILES and args.redownload_script:
                print("Attempting to re-download skipped/corrupt files...")
                cmd = [args.redownload_script] + [
                    p for p in args.redownload_args.split() if p
                ]
                try:
                    subprocess.run(cmd, check=False)
                    print(
                        "Re-download script finished. You may re-run this script to include recovered files."
                    )
                except Exception as e:
    ...
    if SKIPPED_FILES:
        ...
        print(
            f"[Info] Skipped {len(SKIPPED_FILES)} file(s) ({skip_pct:.1f}%). List written to: {log_path}"
        )
        if skip_pct > 5.0:
            print(
                "[Warning] More than 5% of random files were skipped. Consider re-downloading the missing/corrupted files."
            )
