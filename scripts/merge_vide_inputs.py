"""
Prepare and merge DESI EDR data and randoms into structured inputs for VIDE void finder.
"""
...
def main():
    ...
    ap.add_argument(
        ...
        choices=["mpc", "mpch"],
        default="mpc",
        help="Units of inputs and outputs",
    )
    ap.add_argument(
        "--data",
        type=str,
        default=None,
        help="Path to data xyz txt (defaults per unit)",
    )
    ap.add_argument(
        "--randoms",
        type=str,
        default=None,
        help="Path to randoms xyz txt (defaults per unit)",
    )
    args = ap.parse_args()
    unit = args.unit
    ...
    print(f"  Unit: {unit}")
    print(
        f"  Data:    N={n_data:,}  min={vec(d_min)}  max={vec(d_max)}  mean={vec(d_mean)}  std={vec(d_std)}"
    )
    print(
        f"  Randoms: N={n_rand:,}  min={vec(r_min)}  max={vec(r_max)}  mean={vec(r_mean)}  std={vec(r_std)}"
    )
    ...
    manifest = {
        "unit": unit,
        "sources": {"data": data_src, "randoms": rand_src},
        "outputs": {"data": data_dst, "randoms": rand_dst},
        "stats": {
            "data": {
                "N": int(n_data),
                "min": d_min.tolist(),
