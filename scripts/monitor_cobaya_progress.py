"""
Monitor Cobaya .progress file and estimate ETA based on R-1 and/or samples.
"""
...
def estimate_eta(
    rows: List[List[str]],
    col: Dict[str, int],
    target_r1: Optional[float],
    max_samples: Optional[int],
    file_ctime: Optional[float],
) -> Tuple[Optional[float], Dict[str, Optional[Number]]]:
    now = time.time()
    ...
    diag: Dict[str, Optional[Number]] = {
        "r1": r1_vals[-1] if r1_vals else None,
        "n": n_vals[-1] if n_vals else None,
    }
...
def main():
    ap = argparse.ArgumentParser(
        description="Monitor Cobaya .progress and estimate ETA"
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="Target R-1 threshold (e.g. 0.02)",
    )
    ap.add_argument(
        "--max-samples",
        type=int,
        default=None,
        help="Target max_samples (accepted steps)",
    )
    ap.add_argument(
    ...
    while True:
        ...
        if mtime != last_mtime:
            ...
            if not header or not rows:
                ...
                eta, diag = estimate_eta(
                    rows, cols, args.target_rminus1, args.max_samples, ctime
                )
                r1 = diag.get("r1")
                n = diag.get("n")
