"""
This script implements a void finding algorithm based on kNN density estimation,
suitable for survey geometries like DESI. It identifies void centers by growing
clusters from random points, and then uses kNN density estimation to assign
density values to each void center. Finally, it uses a density threshold to
identify voids.
"""
...
def estimate_density_kNN(
    data: np.ndarray, randoms: np.ndarray, k: int = 32
) -> np.ndarray:
...
def grow_empty_sphere(
    ...
    nbr_data.fit(data)
    nbr_rnd.fit(randoms)
    r_data, _ = nbr_data.kneighbors(
        center.reshape(1, -1), n_neighbors=1, return_distance=True
    )
    r_rnd, _ = nbr_rnd.kneighbors(
        center.reshape(1, -1), n_neighbors=128, return_distance=True
    )
...
def suppress_overlaps(
    centers: np.ndarray, radii: np.ndarray, overlap_frac: float = 0.3
) -> np.ndarray:
...
def main():
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.003,
        help="Fraction of lowest-density points as seeds",
    )
    ap.add_argument(
        "--min_sep",
        type=float,
        default=8.0,
        help="Min center separation for seeds (same unit as coords)",
    )
    ap.add_argument(
        "--target_q",
        type=float,
        default=0.99,
        help="Quantile for random-distance cap when growing sphere",
    )
    ap.add_argument(
        "--max_voids", type=int, default=500, help="Maximum number of voids to output"
    )
    ap.add_argument(
        "--rand_subsample",
        type=int,
        default=400000,
        help="If >0 and randoms are larger, subsample to this size",
    )
    args = ap.parse_args()
    ...
    seeds = find_minima_candidates(
        data, logdens, frac=args.seed_frac, min_separation=args.min_sep
    )
    ...
    def _grow_with_prefit(center):
        d_data, _ = nn_data.kneighbors(
            center.reshape(1, -1), n_neighbors=2, return_distance=True
        )
        r0 = float(d_data[0, 0])
        r1 = float(d_data[0, 1]) if d_data.shape[1] > 1 else float(d_data[0, 0])
        r_data = r1 if r0 < 1e-09 else r0
        r_rnd, _ = nn_rnd.kneighbors(
            center.reshape(1, -1), n_neighbors=128, return_distance=True
        )
