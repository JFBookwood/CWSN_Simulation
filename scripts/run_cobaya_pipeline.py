"""
Cobaya VGCF Pipeline Runner with MPI and Seeding Options
"""
...
MODES = {
    "quick": YAML_QUICK,
    "full": YAML_FULL,
    "full_parallel": YAML_FULL_PAR,
    "full_parallel_seeded": YAML_FULL_PAR_SEEDED,
}
...
def main():
    ap = argparse.ArgumentParser(
        description="Launch Cobaya VGCF runs with optional MPI and seeding"
    )
    ap.add_argument("--mode", choices=list(MODES), default="full_parallel_seeded")
    ap.add_argument(
        "--mpi", type=int, default=1, help="MPI ranks (mpiexec -n N); use 1 for no MPI"
    )
    ap.add_argument(
        "--omp",
        type=int,
        default=None,
        help="OMP threads per process (sets OMP_NUM_THREADS)",
    )
    ap.add_argument(
        "--resume",
        action="store_true",
        help="Pass resume behavior (YAMLs already have resume: true where relevant)",
    )
    ap.add_argument(
        "--dry", action="store_true", help="Print commands without executing"
    )
    args = ap.parse_args()
    ...
    if args.mode == "full_parallel_seeded" and (not COVMAT_QUICK.exists()):
        print(
            "Seeded mode requested but quick covmat missing. Running quick run first..."
        )
        ...
        if args.dry:
            ...
            if code != 0:
                print(
                    "ERROR: Quick run failed; cannot proceed to seeded full run.",
                    file=sys.stderr,
                )
                sys.exit(code)
        if not COVMAT_QUICK.exists():
            print(
                "WARNING: Quick covmat still not found; proceeding anyway (MCMC will use 'auto' or proposals)."
            )
