"""
Smoke test for the Cobaya pack for VGCF voids.
"""
...
def interp_template(s_grid, xi0_values):
    ...
    def f(s):
        return np.interp(
            s, s_grid, xi0_values, left=xi0_values[0], right=xi0_values[-1]
        )
...
def main():
    ...
    logp = -0.5 * chi2
    print(
        f"Smoke test OK. chi2={chi2:.3f}, logp={logp:.3f}, ndim={len(dvec)}, shrinkage={z.get('shrinkage_lambda', None)}"
    )
