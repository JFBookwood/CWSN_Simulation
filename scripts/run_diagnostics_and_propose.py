"""
mnu Chain Diagnostics and Proposal Generator for Cobaya
"""
...
def apply_burn_in(
    values: np.ndarray, weights: np.ndarray, frac: float
...
def read_progress_acceptance_and_r(
    prog_path: Path,
...
def analyze_chain(
    ...
    acc_rate, r_prog = read_progress_acceptance_and_r(
        progress_path_for_chain(chain_path)
    )
    ...
    if trim_floor is not None:
        ...
        ax.hist(
            v_b, bins=60, density=True, alpha=0.4, color="tab:blue", label="untrimmed"
        )
        ax.hist(
            v_t,
            bins=60,
        ...
        trimmed_stats = {
            "mean": wmean(v_t, w_t),
            "std": wstd(v_t, w_t),
            "p50": wpercentiles(v_t, w_t, [0.5])[0] if len(v_t) else float("nan"),
        }
    summary = {
        "file": str(chain_path),
        "chain_name": cname,
        ...
        "N": N,
        "mean": mu,
        "median": percs[pcts.index(0.5)],
        "std": sd,
        "p01": percs[pcts.index(0.01)],
        "p05": percs[pcts.index(0.05)],
        "p16": percs[pcts.index(0.16)],
        "p50": percs[pcts.index(0.5)],
        "p84": percs[pcts.index(0.84)],
        "p95": percs[pcts.index(0.95)],
        "p99": percs[pcts.index(0.99)],
        "hpd68_lo": hpd68[0],
        "hpd68_hi": hpd68[1],
        "hpd95_lo": hpd95[0],
        "hpd95_hi": hpd95[1],
        ...
        "N_eff": N_eff,
        "Rhat_split4": rhat,
        "accept_rate_log": acc_rate,
        "Rminus1_log": r_prog,
        "trimmed_mean": trimmed_stats["mean"] if trimmed_stats else None,
        "trimmed_std": trimmed_stats["std"] if trimmed_stats else None,
        "trimmed_p50": trimmed_stats["p50"] if trimmed_stats else None,
    }
    return summary
...
def write_summary_csv_md(summaries: List[Dict], outdir_results: Path):
    ...
    lines = [
        "# Summary mnu chains",
        "",
        "Getrimmte Posterioren sind bedingt durch prior/floor — enge Unsicherheiten können artefaktbedingt sein. Keine Detektionsaussage, bevor Mock-Recovery & Konvergenz-Checks abgeschlossen sind.",
        "",
    ]
    for s in summaries:
        lines += [
            f"## {s['chain_name']}",
            f"- N (post burn-in): {s['N']}",
            f"- mean ± σ: {s['mean']:.6g} ± {s['std']:.3g}",
            f"- median [16%,84%]: {s['p50']:.6g} [{s['p16']:.6g}, {s['p84']:.6g}]",
            f"- HPD68: [{s['hpd68_lo']:.6g}, {s['hpd68_hi']:.6g}], HPD95: [{s['hpd95_lo']:.6g}, {s['hpd95_hi']:.6g}]",
            f"- P(mnu>0.06)={s['P_gt_0.06']:.3f}, P(mnu>0.1)={s['P_gt_0.1']:.3f}",
            f"- tau_int≈{s['tau_int']:.2f}, N_eff≈{s['N_eff']:.0f}, R̂_split4={s['Rhat_split4']:.3f}",
            f"- accept_rate (log): {(s['accept_rate_log'] if s['accept_rate_log'] is not None else 'NA')}",
            "",
        ]
...
def build_suggested_yaml(summaries: List[Dict], outdir_analysis: Path):
    taus = [
        s["tau_int"]
        for s in summaries
        if isinstance(s.get("tau_int"), (int, float)) and (not math.isnan(s["tau_int"]))
    ]
    ...
    for neff in targets:
        ...
        suggestions[neff] = {
            "tau_ref": tau_ref,
            "chains": 4,
            "steps_per_chain": per_chain,
        }
    content = {
        "theory": "camb (unchanged)",
        "sampler": "mcmc",
        "proposal_scale": 0.5,
        "learn_proposal": True,
        "min_accepted_steps": 20000,
        "recommendations": suggestions,
        "notes": [
            "Start 4 independent chains with seeds and diverse mnu starts: [0.005, 0.06, 0.1, 0.2] eV",
            "Aim acceptance rate 0.2–0.4; adjust proposal_scale accordingly",
            "If hard floor 0.059: consider reparam y = log(mnu - 0.059 + 1e-6) and sample y",
        ],
    }
...
def write_sample_requirements(summaries: List[Dict], outdir_results: Path):
    ...
    keys = [
        ...
        "N_eff_current",
        "N_needed_eff2000",
        "extra_needed_eff2000",
        "N_needed_eff10000",
        "extra_needed_eff10000",
    ]
    with path.open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=keys)
        w.writeheader()
        for s in summaries:
            tau = (
                s["tau_int"]
                if isinstance(s.get("tau_int"), (int, float))
                and (not math.isnan(s["tau_int"]))
                else float("nan")
            )
            N = s["N"]
            Neff = (
                s["N_eff"] if isinstance(s.get("N_eff"), (int, float)) else float("nan")
            )
            ...
            row = {
                ...
                "N_current": N,
                "N_eff_current": Neff,
                "N_needed_eff2000": n2000,
                "extra_needed_eff2000": n2000 - N if isinstance(n2000, int) else "",
                "N_needed_eff10000": n10000,
                "extra_needed_eff10000": n10000 - N if isinstance(n10000, int) else "",
            }
            w.writerow(row)


def mh_mock_recovery(
    mnu_true: float,
    floor: Optional[float],
    steps: int = 20000,
    proposal_sigma: float = 0.002,
    seed: int = 0,
) -> Dict:
    ...
    return {
        ...
        "hpd68_lo": hpd68[0],
        "hpd68_hi": hpd68[1],
        "in_hpd68": mnu_true >= hpd68[0] and mnu_true <= hpd68[1],
...
def write_mock_outputs(outdir_results: Path, floor: Optional[float]):
    ...
    for mtrue in [0.06, 0.1]:
        res = mh_mock_recovery(
            mtrue, floor=floor, steps=30000, proposal_sigma=0.002, seed=42
        )
        rows.append(res)
...
def write_pdf_report(outdir_results: Path, summaries: List[Dict]):
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        fig.text(
            0.1,
            0.86,
            "Getrimmte Posterioren sind bedingt durch prior/floor — enge Unsicherheiten können artefaktbedingt sein.",
            fontsize=9,
        )
        fig.text(
            0.1,
            0.83,
            "Keine Detektionsaussage, bevor Mock-Recovery & Konvergenz-Checks abgeschlossen sind.",
            fontsize=9,
        )
        ...
        for s in summaries:
            ...
            line(f"File: {s['file']}")
            line(
                f"N={s['N']}, mean={s['mean']:.6g}, std={s['std']:.3g}, median={s['p50']:.6g}"
            )
            line(
                f"HPD68=[{s['hpd68_lo']:.6g}, {s['hpd68_hi']:.6g}], HPD95=[{s['hpd95_lo']:.6g}, {s['hpd95_hi']:.6g}]"
            )
            ...
            line(
                f"accept_rate_log: {(s['accept_rate_log'] if s['accept_rate_log'] is not None else 'NA')}"
            )
            pdf.savefig(fig)
            plt.close(fig)
...
def main():
    ...
    ap.add_argument(
        ...
        action="append",
        required=True,
        help="Path to Cobaya chain .txt (can pass multiple)",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.059,
        help="For overlay and trimmed stats; set None to disable",
    )
    args = ap.parse_args()
