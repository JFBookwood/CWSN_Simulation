"""
Quality Control for Void Catalogs using kNN Density Profiles
"""
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=500000,
        help="Subsample randoms for speed (0=disable)",
    )
    ...
    radii = vdf[[c for c in vdf.columns if c.startswith("radius_")][0]].to_numpy(
        dtype=np.float64
    )
    ...
    for i, (c, rv) in enumerate(zip(centers, radii)):
        ...
        rmax = x_max * rv
        d_g, _ = nn_g.radius_neighbors(
            c.reshape(1, -1), radius=rmax, return_distance=True
        )
        if d_g[0].size > 0:
            xg = d_g[0] / rv
            hg, _ = np.histogram(xg, bins=edges)
            num_g += hg.astype(np.int64)
        d_r, _ = nn_r.radius_neighbors(
            c.reshape(1, -1), radius=rmax, return_distance=True
        )
        if d_r[0].size > 0:
            xr = d_r[0] / rv
