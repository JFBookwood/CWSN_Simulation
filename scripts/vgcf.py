"""
Void-Galaxy Correlation Function Calculation
"""
...
def main():
    ...
    ap.add_argument(
        "--smax", type=float, default=120.0, help="Max separation for pairs"
    )
    ...
    for i, c in enumerate(centers):
        ...
        los = unit_vector(c)
        dg, idx_g = nn_g.radius_neighbors(
            c.reshape(1, -1), radius=args.smax, return_distance=True
        )
        if idx_g[0].size:
            vec = gal[idx_g[0]] - c
            ...
            if np.any(mask):
                ...
                NG += H
        dr, idx_r = nn_r.radius_neighbors(
            c.reshape(1, -1), radius=args.smax, return_distance=True
        )
        if idx_r[0].size:
            vec = ran[idx_r[0]] - c
    ...
    out = pd.DataFrame(
        {
            "s_center_" + args.unit: s_centers,
            "xi0": xi0,
            "xi2": xi2,
            "N_pairs_g": NG.sum(axis=1),
            "N_pairs_r": NR.sum(axis=1),
            "N_voids": len(centers),
        }
    )
    os.makedirs(OUT_DIR, exist_ok=True)
    ...
    try:
        ...
        print(
            f"[Info] Matplotlib not available or plotting failed ({e}); CSV written.)"
        )
