"""
Jackknife computation of void-galaxy correlation function multipoles.
"""
...
def compute_multipoles(gal, ran, centers, s_edges, mu_edges):
    ...
    for i, c in enumerate(centers):
        if i % max(1, len(centers) // 10) == 0:
            pass
        los = unit_vector(c)
        _, idx_g = nn_g.radius_neighbors(
            c.reshape(1, -1), radius=s_edges[-1], return_distance=True
        )
        if idx_g[0].size:
            vec = gal[idx_g[0]] - c
